...
function r2(){localSubmitted=false;let src=state.players.find(p=>p.id===state.a2[me.id]);let q=state.s1[src.id].ingredient;render(`<div class="top"><div><span class="pill">ROUND 2 / 3</span></div><div id="timer" class="timer"></div></div><div class="panel"><div class="hint">Another chef's challenge:</div><div class="prompt">🥕 ${esc(q)}</div><div class="field"><label>Exactly 6 ingredients</label><textarea id="list" placeholder="One per line"></textarea></div><div class="actions"><button class="btn primary" id="go">Lock in six</button></div><div id="players" class="players"></div></div>`);$("#go").onclick=submit2;startTimer(submit2)}
async function submit2(expired=false){if(localSubmitted)return;let a=($("#list")?.value||"").split(/[\\n,]+/).map(x=>x.trim()).filter(Boolean);if(!expired&&a.length!==6)return toast("Enter exactly 6 ingredients");while(a.length<6)a.push(rand(FALLBACK));localSubmitted=true;stopTimer();const p={id:me.id,ingredients:a.slice(0,6)};if(me.host){state.s2[me.id]=p;send("state",state);updatePlayers();if(all(state.s2))round3();else wait("Waiting for everyone to finish Round 2…")}else{await send("s2",p);wait("Submitted! Waiting for everyone to finish Round 2…")}}
