...
let db=null,ch=null,state=null,timer=null;
...
async function publish(){await send('state',clone())}
...
async function onMessage(m){
 ...
 if(m.type==='state'){
   ...
   const old=state?.phase;state=incoming;
   // Crucial: if another player submits while I am typing, update the player
   // list but NEVER redraw my current form or restart my timer.
   if(old!==state.phase){stop();renderCurrent()}
...
async function start(host){
 ...
 state={room,phase:'lobby',timer:90,players:[{id:me.id,name:me.name,ready:false}],s1:{},s2:{},s3:{},a2:{},a3:{},votes:{}};
