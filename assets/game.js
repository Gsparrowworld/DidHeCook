...
function r2(){localSubmitted=false;let src=state.players.find(p=>p.id===state.a2[me.id]);let challenge=state.s1[src.id];let q=challenge.ingredient;render(`<div class="top"><div><span class="pill">ROUND 2 / 3</span></div><div id="timer" class="timer"></div></div><div class="panel"><div class="hint">Another chef's ingredient challenge:</div><div class="prompt">🥕 ${esc(q)}</div><div class="hint">Think of six different ingredients that fit this prompt. Type one ingredient in each box.</div><div class="ingredient-grid">${Array.from({length:6},(_,i)=>`<div class="ingredient-slot"><label>Ingredient ${i+1}</label><input class="ing-input" data-ing="${i}" maxlength="30" autocomplete="off" placeholder="Type ingredient ${i+1}"></div>`).join("")}</div><div class="actions"><button class="btn primary" id="go">Lock in six ingredients</button></div><div id="players" class="players"></div></div>`);$("#go").onclick=submit2;startTimer(submit2)}
...
function r3(){localSubmitted=false;let a=state.a3[me.id];render(`<div class="top"><div><span class="pill">ROUND 3 / 3</span></div><div id="timer" class="timer"></div></div><div class="panel"><div class="hint">Your final cooking challenge:</div><div class="prompt">🍽️ ${esc(a.food)}</div><div class="hint">You have been given 8 ingredients from other chefs. <b>Select the ingredients you want to use.</b></div><div id="ingredientChoices" class="choice-grid">${a.ingredients.map((x,i)=>`<label class="choice"><input type="checkbox" class="dish-ing" value="${esc(x)}" data-i="${i}"><span>${esc(x)}</span></label>`).join("")}</div><div class="status">Select at least 1 ingredient from the eight supplied ingredients.</div><div class="field"><label>Optional chef's pitch</label><textarea id="recipe" placeholder="Explain your creation to the judges..."></textarea></div><div class="actions"><button class="btn primary" id="go">Serve my dish</button></div><div id="players" class="players"></div></div>`);$("#go").onclick=submit3;startTimer(submit3)}
...
async function round3(){state.phase="r3";state.a3={};state.players.forEach(p=>{let others=state.players.filter(x=>x.id!==p.id);let foodSource=rand(others);let ingredientSources=others.filter(x=>state.s2[x.id]?.ingredients?.length);let pool=[];ingredientSources.forEach(x=>pool.push(...state.s2[x.id].ingredients));pool=[...new Set(pool)];while(pool.length<8)pool.push(rand(FALLBACK));pool=[...new Set(pool)];while(pool.length<8)pool.push(FALLBACK[pool.length%FALLBACK.length]);state.a3[p.id]={ingredients:pool.slice(0,8),food:state.s1[foodSource.id].food}});await send("state",state);draw()}
