...
function r2(){localSubmitted=false;let src=state.players.find(p=>p.id===state.a2[me.id]);let challenge=state.s1[src.id];let q=challenge.ingredient;let fields=Array.from({length:6},(_,i)=>'<div class="ingredient-slot"><label>Ingredient '+(i+1)+'</label><input type="text" class="ing-input" data-ing="'+i+'" maxlength="30" autocomplete="off" placeholder="Type ingredient '+(i+1)+'"></div>').join("");render('<div class="top"><div><span class="pill">ROUND 2 / 3</span></div><div id="timer" class="timer"></div></div><div class="panel"><div class="hint">ANOTHER CHEF\'S INGREDIENT CHALLENGE</div><div class="prompt">🥕 '+esc(q)+'</div><div class="hint"><b>TYPE SIX DIFFERENT INGREDIENTS.</b><br>Use all six separate boxes below. The food prompt is intentionally NOT shown until Round 3.</div><div class="ingredient-grid">'+fields+'</div><div class="actions"><button class="btn primary" id="go">Lock in six ingredients</button></div><div id="players" class="players"></div></div>');$("#go").onclick=submit2;startTimer(submit2)}
async function submit2(expired=false){if(localSubmitted)return;let fields=[...document.querySelectorAll(".ing-input")];let a=fields.map(x=>x.value.trim());if(!expired){if(a.some(x=>!x))return toast("Fill in all 6 ingredient boxes");if(new Set(a.map(x=>x.toLowerCase())).size!==6)return toast("Each ingredient must be different")}a=a.map((x,i)=>x||FALLBACK[i%FALLBACK.length]);localSubmitted=true;stopTimer();const p={id:me.id,ingredients:a.slice(0,6)};if(me.host){state.s2[me.id]=p;send("state",state);updatePlayers();if(all(state.s2))round3();else wait("Waiting for everyone to finish Round 2…")}else{await send("s2",p);wait("Submitted! Waiting for everyone to finish Round 2…")}}
function r3(){localSubmitted=false;let a=state.a3[me.id];let choices=a.ingredients.map((x,i)=>'<label class="choice"><input type="checkbox" class="dish-ing" value="'+esc(x)+'" data-i="'+i+'"><span>'+esc(x)+'</span></label>').join("");render('<div class="top"><div><span class="pill">ROUND 3 / 3</span></div><div id="timer" class="timer"></div></div><div class="panel"><div class="hint">YOUR FINAL COOKING CHALLENGE</div><div class="prompt">🍽️ '+esc(a.food)+'</div><div class="hint"><b>CHOOSE INGREDIENTS FOR THIS DISH.</b><br>These 8 ingredients came from other players. Click the ones you want to use.</div><div class="choice-grid">'+choices+'</div><div class="status">Select at least 1 of the 8 ingredients.</div><div class="field"><label>Optional chef\'s pitch</label><textarea id="recipe" placeholder="Explain your creation to the judges..."></textarea></div><div class="actions"><button class="btn primary" id="go">Serve my dish</button></div><div id="players" class="players"></div></div>');$("#go").onclick=submit3;startTimer(submit3)}
...
async function round3(){state.phase="r3";state.a3={};state.players.forEach(p=>{const others=state.players.filter(x=>x.id!==p.id);const foodSource=rand(others);let pool=[];others.forEach(x=>{const ans=state.s2[x.id]?.ingredients||[];pool.push(...ans)});pool=[...new Set(pool)];for(let i=pool.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[pool[i],pool[j]]=[pool[j],pool[i]]}for(const x of FALLBACK){if(pool.length>=8)break;if(!pool.includes(x))pool.push(x)}state.a3[p.id]={ingredients:pool.slice(0,8),food:state.s1[foodSource.id].food}});await send("state",state);draw()}
