...
function r2(){localSubmitted=false;let src=state.players.find(p=>p.id===state.a2[me.id]);let q=state.s1[src.id].ingredient;render(`<div class="top"><div><span class="pill">ROUND 2 / 3</span></div><div id="timer" class="timer"></div></div><div class="panel"><div class="hint">Another chef's challenge:</div><div class="prompt">🥕 ${esc(q)}</div><div class="field"><label>Select exactly 6 ingredients</label><div id="ingredientFields" class="ingredient-grid">${Array.from({length:6},(_,i)=>`<div class="ingredient-slot"><label>Ingredient ${i+1}</label><select class="ing-select" data-ing="${i}"><option value="">Choose an ingredient…</option></select></div>`).join("")}</div></div><div class="actions"><button class="btn primary" id="go">Lock in six</button></div><div id="players" class="players"></div></div>`);const choices=[...new Set([...FALLBACK,"bacon","chicken","beef","pork","fish","shrimp","sausage","ham","tofu","beans","rice","noodles","bread","flour","oats","egg","milk","cream","butter","cheese","yogurt","tomato","potato","carrot","onion","garlic","mushrooms","spinach","lettuce","broccoli","celery","corn","capsicum","avocado","cucumber","apple","banana","strawberry","mango","pineapple","lemon","lime","ginger","honey","sugar","chocolate","peanut butter","olive oil","soy sauce","mustard","ketchup","hot sauce","pickles","cinnamon","chili"] )];document.querySelectorAll(".ing-select").forEach(sel=>{sel.innerHTML+=choices.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join("");sel.addEventListener("change",()=>{const used=new Set([...document.querySelectorAll(".ing-select")].filter(x=>x!==sel).map(x=>x.value));document.querySelectorAll(".ing-select").forEach(x=>[...x.options].forEach(o=>{o.disabled=o.value!==""&&used.has(o.value)&&x!==sel}))})});$("#go").onclick=submit2;startTimer(submit2)}
...
function r3(){localSubmitted=false;let a=state.a3[me.id];render(`<div class="top"><div><span class="pill">ROUND 3 / 3</span></div><div id="timer" class="timer"></div></div><div class="panel"><div class="hint">Use all eight ingredients to make:</div><div class="prompt">🍽️ ${esc(a.food)}</div><div>${a.ingredients.map(x=>`<span class="ingredient">${esc(x)}</span>`).join("")}</div><div class="field"><label>Recipe / pitch</label><textarea id="recipe" placeholder="Tell the judges why this dish is incredible"></textarea></div><div class="actions"><button class="btn primary" id="go">Serve it!</button></div><div id="players" class="players"></div></div>`);$("#go").onclick=submit3;startTimer(submit3)}
async function submit3(expired=false){if(localSubmitted)return;let recipe=$("#recipe")?.value.trim();if(!expired&&!recipe)return toast("Write your recipe");let a=state.a3[me.id];localSubmitted=true;stopTimer();const p={id:me.id,recipe:recipe||"The chef's special.",food:a.food,ingredients:a.ingredients};if(me.host){state.s3[me.id]=p;send("state",state);updatePlayers();if(all(state.s3))voting();else wait("Waiting for everyone to serve…")}else{await send("s3",p);wait("Served! Waiting for everyone to finish…")}}
